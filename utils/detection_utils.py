...
def visualize_boxes_and_labels_on_image_array(image, 
    ...
    for i in range(len(boxes)):
        ...
        if variance is not None:
            variance_string = ','.join(str(round(x,2)) for x in variance[i].tolist())
            variance_str='[' +variance_string+ ']'
            display_str = "{}: {}%: {}".format(display_str, round(100 * scores[i]),variance_str)
        else:
