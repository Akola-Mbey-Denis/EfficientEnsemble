...
class KITTIObjectDetectYOLO(Dataset):
    """
    COCO dataset class.
    """
    def __init__(self, model_type, data_dir='/home/denis/Downloads/KITTI_COCO', json_file='labels.json',
                 name='train', img_size=416,
                 augmentation=None, min_size=1, debug=False,evaluate=False):
        """
        ...
        """
        # json_file="test_cocoformat.json"  if name=="test" else "train_cocoformat.json"
        ...
        # self.label ="labels"
        print(os.path.join(self.data_dir, self.json_file))
        self.path =os.path.join(self.data_dir, self.json_file)
        ...
        # self.dataset=self.coco.dataset['videos']
    ...
    def __getitem__(self, index):
        ...
        # print("Image :::",self.coco.loadImgs(ids=[id_])[0]['file_name'])
        # image_info = self.coco.loadImgs(ids=[id_])[0]['file_name']
        # img_file = os.path.join(self.data_dir,self.name, image_info)

        file_name = (str(index).zfill(6) + '.png')

        img_file = os.path.join(self.data_dir,'data',file_name)
        # print(img_file)
        ...
        if len(labels) > 0:
            ...
            if 'YOLO' in self.model_type:
                if not self.evaluate:
                    ...
                    padded_labels =torch.from_numpy(labels)
